...
class HighPassFilter:
    def __init__(self, alpha=0.9):
        ...
        self.prev_x = np.zeros(3)  # 前一时刻的输入
        self.prev_y = np.zeros(3)  # 前一时刻的输出
...
# 初始化EKF和高通滤波器
ekf = EKF()
hp_filter = HighPassFilter(alpha=0.95)  # alpha可调，0.9为示例
...
while True:
    if imu.IMURead():
        ...
        # 应用高通滤波
        filtered_lin_accel = hp_filter.filter(lin_accel)

        # 输出滤波后的线性加速度
        print("滤波后线性加速度 (g): X=%.3f, Y=%.3f, Z=%.3f" % tuple(filtered_lin_accel))
